...
for i in range(0, len(calls)+1):
	winners = [board for board in boards if winning_board(calls[:i], board)]
	if len(winners) > 0:
		print('We have a winner: %s' % winners)
		#score = sum(set([item for sublist in winners[0] for item in sublist]) - set(calls[:i])) * calls[:i][-1]
		score = score_board(calls[:i], winners[0])
